    ...
    public partial class MainPage : ContentPage
    {
        ...
        Grid grid = new Grid();
            ...
            for (int i = 0; i < cantidadFilas; i++)
            {
                ...
                grid.Add(new Label { Text = "1", VerticalOptions = LayoutOptions.Center, HorizontalOptions = LayoutOptions.Center }, 0, i); // Columna 1
                grid.Add(fila[0], 1, i); // Columna 2
                grid.Add(fila[1], 2, i); // Columna 3
                grid.Add(fila[2], 3, i); // Columna 4
                grid.Add(new Frame { Content = CreateFrameContent(i, fila), HasShadow = false, BorderColor = Color.FromRgb(0,0,0) }, 4, i); // Columna 5
            }
        ...
        private void OnNextClicked(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                for(int i = 0; i < alternativasG; i++)
                {
                    float[] promactual = tablasGlobal[i + 1].promedioFilas;
                    for (int j = 0; j < criteriosG; j++)
                        {
                            matriz[i, j] = promactual[j];
                        }
                    }
                }
            }
